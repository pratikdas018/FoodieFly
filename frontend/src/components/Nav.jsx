...
function Nav() {
    ...
    const iconButtonClass = "h-[36px] w-[36px] max-[360px]:h-[33px] max-[360px]:w-[33px] max-[320px]:h-[31px] max-[320px]:w-[31px] md:h-[40px] md:w-[40px] rounded-full border border-orange-200 bg-white text-[#ff4d2d] flex items-center justify-center shadow-sm hover:bg-orange-50 transition-all duration-200 cursor-pointer"
    ...
    const mobileSpacerClass = userData?.role === "user" ? "h-[126px] max-[360px]:h-[120px] max-[320px]:h-[116px] md:h-[84px]" : "h-[72px] max-[360px]:h-[68px] max-[320px]:h-[64px] md:h-[84px]"
    ...
    return (
      <>
        <div className='fixed top-0 left-0 w-full z-[9999] bg-gradient-to-r from-[#fff9f6]/95 via-white/95 to-[#fff3eb]/95 backdrop-blur-md border-b border-orange-100 shadow-[0_10px_30px_rgba(255,77,45,0.12)] overflow-visible'>
          <div className='max-w-[1200px] mx-auto px-3 max-[360px]:px-2.5 md:px-6 py-2 md:py-0 md:h-[84px] flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-3 relative'>
            <div className='w-full md:w-auto flex items-center justify-between gap-1.5 md:gap-2'>
              <button type="button" className='flex items-center gap-2 max-[360px]:gap-1.5 md:gap-3 shrink min-w-0 cursor-pointer' onClick={handleNavigateHome}>
                <div className='nav-brand-badge h-8 w-8 max-[360px]:h-7 max-[360px]:w-7 max-[320px]:h-[26px] max-[320px]:w-[26px] md:h-9 md:w-9 rounded-xl bg-gradient-to-br from-[#ff4d2d] to-[#ff7b5d] text-white font-black flex items-center justify-center shadow-md text-sm max-[320px]:text-xs'>
                  F
                </div>
                <h1 className='nav-brand-title text-xl max-[360px]:text-[18px] max-[320px]:text-[16px] sm:text-2xl md:text-3xl font-black tracking-tight bg-gradient-to-r from-[#ff4d2d] to-[#ff7b5d] bg-clip-text text-transparent truncate leading-none max-w-[130px] max-[360px]:max-w-[110px] max-[320px]:max-w-[96px] md:max-w-none'>
                  FoodieFly
                </h1>
              </button>

              <div className='flex items-center gap-1 max-[360px]:gap-0.5 md:gap-3 relative shrink-0'>
                {userData.role == "owner" ? <>
                  {myShopData && <>
                  ...
                  <>
                    {userData.role == "user" && <button className={`relative ${iconButtonClass}`} onClick={() => navigate("/cart")} title='Cart'>
                    ...
                    {userData.role == "user" && <button className={`relative ${iconButtonClass} hidden md:flex`} onClick={handleNavigateToFavorites} title='Favorites'>
                      <FiHeart size={18} />
                      {favoriteCount > 0 && (
                ...
                <button className='nav-profile-btn w-[36px] h-[36px] md:w-[40px] md:h-[40px] rounded-full flex items-center justify-center bg-gradient-to-br from-[#ff4d2d] to-[#ff7b5d] text-white text-[15px] md:text-[16px] shadow-lg font-semibold cursor-pointer border-2 border-white' onClick={() => { setShowNotifications(false); setShowInfo(prev => !prev) }}>
                ...
                {showInfo && <div className='absolute top-[52px] right-0 w-[220px] max-w-[calc(100vw-20px)] bg-white/95 backdrop-blur-md border border-orange-100 shadow-2xl rounded-2xl p-[16px] flex flex-col gap-[10px] z-[9999]'>
                  ...
                  {userData.role == "user" && <button className='md:hidden text-left text-[#ff4d2d] font-semibold cursor-pointer hover:text-[#e64526]' onClick={handleNavigateToFavorites}>Favorites</button>}
                  ...
                  <button className='text-left text-[#ff4d2d] font-semibold cursor-pointer hover:text-[#e64526]' onClick={handleThemeToggle}>Toggle Theme</button>
            ...
            {userData.role == "user" && <div className='w-full md:w-[58%] lg:w-[44%] h-[48px] max-[360px]:h-[44px] max-[320px]:h-[42px] md:h-[60px] rounded-2xl border border-orange-100 bg-white/90 shadow-[0_8px_20px_rgba(255,77,45,0.12)] items-center gap-2 max-[360px]:gap-1 flex px-3 max-[360px]:px-2'>
              <div className='flex items-center w-[34%] max-[360px]:w-[40%] max-[320px]:w-[44%] overflow-hidden gap-[8px] max-[360px]:gap-[6px] px-[8px] max-[360px]:px-[6px] border-r border-orange-200'>
                <FaLocationDot size={16} className="text-[#ff4d2d] shrink-0" />
                <div className='w-[80%] truncate text-gray-700 text-xs max-[320px]:text-[10px] md:text-sm font-medium'>{currentCity || "Your city"}</div>
              </div>
              <div className='w-[66%] max-[360px]:w-[60%] max-[320px]:w-[56%] flex items-center gap-[8px] max-[360px]:gap-[5px]'>
                <IoIosSearch size={18} className='text-[#ff4d2d] shrink-0' />
                <input type="text" placeholder='search food...' className='px-[2px] text-gray-700 outline-0 w-full text-xs max-[320px]:text-[10px] md:text-sm placeholder:text-gray-400' onChange={(e) => setQuery(e.target.value)} value={query} />
              </div>
            </div>}
          </div>
        </div>
        <div className={mobileSpacerClass}></div>
      </>
    )
}
